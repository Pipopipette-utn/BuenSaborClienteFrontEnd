  ...
  return (
    <Modal open={open} onClose={handleClose}>
      <Box sx={style}>
        ...
        <Typography sx={{ mt: 2 }}>{articulo.descripcion}</Typography>
        <Typography sx={{ mt: 2 }}>Precio: ${articulo.precioVenta}</Typography>
        <Box sx={{ mt: 2, display: "flex", alignItems: "center" }}>
          <Button variant="contained" onClick={handleDecrement} sx={{ mr: 2 }}>
            -
          </Button>
          <Typography variant="body1">{cantidad}</Typography>
          <Button variant="contained" onClick={handleIncrement} sx={{ ml: 2 }}>
            +
          </Button>
        </Box>
